...
% Single-blind review: authors are visible, so NOT the anonymising "review"
% option. "preprint" keeps the byline and adds page numbers for reviewers.
...
\begin{abstract}
This paper describes a closed-book system for the LM-KBC 2026 shared task, built
on a single 31.3B parameter base model in completion mode with no training and no
retrieval. It scores 0.7060 and is ahead on five of six relations.

The largest single improvement I found is not an algorithm. Asking the model to
recall facts about an entity before committing to an answer, rather than asking
for the answer directly, moved \texttt{hasArea} from 0.8100 to 0.8700. Three
earlier configurations, across two frames and two aggregation methods, had all
returned exactly 0.8100, which I had read as evidence that the answers were
absent from the candidate pool. That reading was incorrect.
...
vote frequency then agree that the relation is knowledge bound.
\end{abstract}
...
Two features of the scoring decide what a good system looks like, and both work
...
all of them, and a system that abstains everywhere still scores 0.2147. The
abstention decision is therefore worth more than additional recall on the rows
that do have answers.

Second, the two numeric relations are graded as a single value within five
percent of the gold. There is no partial credit and no set overlap available.
Together the numeric relations are 198 of 475 rows, so a system's numeric
behaviour accounts for close to half its score.
...
\texttt{hasCapacity} is no longer present. I report the retraction in full,
including the effort spent on the wrong side of it, because the same
uncontrolled statistic is what the rest of this literature uses to direct its
effort.

Code, prompts, seeds, threshold grids, and the parameter audit are available at
\url{https://github.com/thylinao1/lm-kbc-2026}.
...
Those rates are measured rather than assumed. I submitted a single all empty
...
Two consequences are used throughout. \texttt{awardWonBy} is 2.1\% of the score
across ten rows, so its relation level F1 carries a confidence interval of
roughly $\pm 0.23$ and work on it cannot be measured reliably. And the validation
...
This system builds on published work, and the parts that are replication are
identified as such.
...
the design here: base completion performs better than instruction tuned on most
relations, and a standalone abstention gate was measurably inert while removing
vote share carried a large cost. I reimplement the recipes from the published
description rather than copying code, because that repository carries no licence
file. A second public system \cite{dukesun2026} reaches a lower overall score
using one token logit probes with placebo correction, multi channel numeric
elicitation, and cross model agreement. It is CC BY 4.0 and I credit it for the
multi channel numeric idea and the overshoot calibration.

From the 2025 edition \cite{kalo2025lmkbc,lmkbc2024} I take entity level self
consistency \cite{wang2023selfconsistency} with per relation thresholds, the
finding that chain of thought reduces factual recall on several relations, and a
decomposition and union strategy for the high cardinality award relation,
together with the negative result that decomposition reduces performance on
medium cardinality relations.
...
served with vLLM. I measured the checkpoint at 31,273,088,876 BF16 parameters
from the safetensors index, which is under the 32B cap. For auditors, the model's
hosting page displays 32.68B; the difference is the tied input embedding counted
a second time as an output projection, and the served model holds one copy.

The budget is a hard constraint on architecture rather than a preference. At
31.273B the remaining headroom is 0.727B, which excludes any second neural
component: no agreement sidecar, no reranker, no draft model for speculative
decoding. Every component of the system after generation is non neural post
processing. I also require that any submitted predictions file is produced end to
end by this single substrate, since mixing per relation outputs from two models
would sum their parameter counts and breach the cap without this being visible in
the output file.
...
more prefill for a gain that sits inside the measurement noise here.
...
For numeric relations I score each distinct drawn value by how many draws fall
within the grader's own tolerance of it and take the argmax, emitting a single
bare integer. Averaging across clusters is not appropriate here: a draw set of
4900, 5000, 5050, 5100 and 50000 has a mean near 14000, which fails under any
tolerance, and a largest cluster median of 5025, which is correct.
...
sits beside it, so a stale pool cannot be reused without this being detected.
Draw count is deliberately excluded from the key, and draws are append only with
per draw seeds, so raising the number of draws extends a pool instead of
invalidating it.
...
only the three official split files plus generated artifacts and is enforced in
code by a single loader, a fork hygiene rule that keeps other participants'
repositories outside the published tree and copies no file from them, and offline
environment variables on every inference job, because the compute nodes used here
...
\subsection{Threshold selection, and two effects that inflate it}

Thresholds are chosen on train and reported on validation. Two effects had to be
corrected before the numbers here were reliable, and both inflated results in the
direction of appearing better.
...
answer in its prompt. For borders that was 32 of 67 subjects, and it inflated the
train figure from 0.9854 to 0.9924. I exclude demonstration subjects from the
train curve. The cost is statistical power: 35 scorable subjects remain, so train
side threshold choice on small relations is weak, and I report this rather than
relying on it.
...
edge. I take the centre of the widest tied run instead. On borders that moves the
choice from 0.15 to 0.30 and validation from 0.9846 to 0.9896.

\subsection{The status of the validation split}

Validation is a selection set rather than a clean holdout. Every keep or cut
decision reads it. With roughly twenty looks and a paired delta standard
deviation near 0.008, about one spurious keep carrying 0.015 to 0.019 of
validation gain is expected. That inflates the validation number only: the
submitted entry is chosen by observed test score, so the cost is submissions
spent discovering it rather than points lost.

\section{Results}

All figures below are measured on the test set by the official grader rather than
...
\begin{tabular}{lrrrr}
\toprule
relation & rows & this & best other & diff \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Test results by relation.}
\label{tab:results}
\end{table}

The exception, \texttt{awardWonBy}, has ten rows and a relation-level confidence
interval of roughly $\pm 0.23$, so it is not measurable at this size and I did
not spend effort on it.

\subsection{Elicitation register rather than aggregation}

The single change that produced the largest improvement in this work is not an
...
entity is moved the relation from 0.8100 to 0.8700 on test, above the previous
best posted figure of 0.8500. The same register is the best frame here for
\texttt{hasCapacity}, where it reaches a pool coverage of 0.804 against the 0.77
reported by the strongest published system.

This is worth reporting because of how difficult it was to detect. Three
configurations of \texttt{hasArea}, spanning two prompt frames and two
aggregation methods, returned exactly 0.8100 on test. I had concluded from that
stability, and from the three frames agreeing on 90 of 100 subjects, that the
incorrect rows were incorrect because the pool did not contain the answer. That
conclusion was wrong. Pool composition depends on the register, and better
selection from a poorly elicited pool does not recover what the register never
surfaced. Validation did not detect this: it rated the two frames a tie, 0.8500
each, with a paired bootstrap interval of $[-0.050, +0.050]$.

\subsection{Validation alone could not have produced this system}
...
and cost $-0.0102$. Validation did not detect the two largest improvements,
understated a third by half, and inverted the sign of the remaining two.

The reason is arithmetic rather than chance: each relation has 67 to 100 rows, so
its validation standard error is 0.03 to 0.05, while the differences that decide
the ranking are 0.01 to 0.03. A validation split of this size cannot resolve
them, and a system tuned only on it is tuned on noise.
...
0.6916. I report this as a methodological finding rather than a technique.
Calibrating scalar decision parameters on returned aggregate per-relation scores
is disclosed here as part of the method. I did not, and would not, use per-row
feedback or any procedure that reconstructs which specific rows are correct.

\subsection{A negative result with high confidence}
...
borders threshold (both 0.0000). Every scalar axis under my control is therefore
at a measured local optimum, and \texttt{cityOfDeath} is bracketed on both sides
(0.5800 at 0.35, 0.6100 at 0.45, 0.6000 at 0.55).

Summing the best posted figure for each relation separately gives 0.7015, which
is only 0.0054 above the leading single system. A system built by assembling
published recipes therefore cannot exceed that system by more than noise.
Exceeding this envelope requires improving on a per relation best somewhere,
which is why the work concentrated on one relation.

\subsection{\texttt{hasCapacity} is not selection bound, although the coverage
metric indicates that it is}
\label{sec:coverage}

For each subject I separate two questions. Coverage asks whether any draw in the
pool contains an acceptable answer, which bounds what any selector reading that
pool could achieve. Realized asks whether the aggregator actually emitted one.
...
That reading is incorrect, and the rest of this section is the correction. I
report it in full because I accepted it for most of the work and spent most of
the available effort on it.
...
differs is how much of the headline number reflects that knowledge. Area draws
are close to unanimous, so area coverage is almost entirely knowledge, and rank
one selection already realises 0.850 of the available 0.940. Capacity draws are
dispersed, and 35.5 of the 84.5 coverage points reflect the pool being wide
enough to contain many plausible values.

Decomposing by where the gold sits in the frequency ranking indicates where a
selector could find something. Excess is the real rate minus the coincidence rate
at that rank: at rank 1 it is $0.330 - 0.049 = +0.281$; at ranks 2 to 3,
$0.216 - 0.077 = +0.140$; at ranks 4 to 10, $0.206 - 0.160 = +0.046$; at rank 11
or worse, $0.082 - 0.066 = +0.017$.
...
number I, and the published system I was working from, had been quoting.

This also accounts for an observation that previously had no explanation. Ranks
four and worse are almost entirely coincidence, and no feature can mark a value
as correct when its presence in the pool is an accident of spread. That is the
mechanical reason why more than twenty published aggregation mechanisms and ten
of my own reach the same limit. They were competing for headroom that is largely
not present. Any oracle style ceiling in closed book knowledge base construction
needs this control before it is used to direct effort.

\subsection{What did not work, and three faults that cost more}

Three selection methods that might have been expected to help did not, and the
reasoning behind each is the more useful contribution. Cross-frame consensus
scored below the best single frame (0.2887 against 0.3196) because the frames
share one base model and one prior toward round numbers, so their errors are
correlated and they agree on the same incorrect attractors. Routing by draw
concentration also scored lower (0.2784 raw, 0.3093 calibrated) because only
about a quarter of frame-subject pairs reach support above 0.6, so the cross
frame choice is made precisely where no signal exists. And the overshoot scaling
reported by a published system did not transfer: all five numeric frames selected
a scale of exactly 1.0.

Separately, three implementation faults each cost more than any modelling
decision made here, and all three left every self-check passing: integer rounding
that removed sub-unit areas, single-linkage clustering that chained across
...
\subsection{Measured priors and assumed priors}

A single all-empty submission provides more information than it appears to.
Because a row with empty gold and an empty prediction scores 1.0, the per
relation macro-F1 of an all-empty file equals that relation's empty-gold rate
exactly, so one submission recovers all six test priors.
...
five point shift, and they move in opposite directions. I re-centre each
...
non-empty groups is unchanged and only their proportion differs, which is a
weaker assumption than treating the splits as interchangeable.

\subsection{Where the remaining headroom is located}
...
the entire field scores below 0.33. I tested this conclusion rather than relying
on it. Forced choice duels are the one instrument available here that is not a
transformation of the sampled frequency table. I generated every pairwise
...
0.3608, so it does not improve on the shipped selector.

The diagnostics are more informative than the verdict, because they distinguish a
broken instrument from a working instrument reporting an absence. The probe is
not degenerate. Position bias is corrected to a mean $P(A)$ of 0.5188, a third of
...
time, at $z$ of 6.0. The relevant question is what that accuracy consists of. On
the same pairs, always choosing the larger of the two numbers scores 60.3
percent, and the probability the model assigns to the larger option rises
monotonically with the ratio between them, from 0.567 below $10^{0.15}$ to 0.727
above $10^{0.75}$. That pattern is consistent with a magnitude prior rather than
with recall. The comparative knowledge above that prior is 2.1 points at $z$ of
0.7.
...
gate. The probe therefore discriminates between candidates, but in the direction
opposite to the one required.
...
same candidate set, agree.}
\label{tab:instruments}
\end{table}

A fourth line of evidence supports this. Sampled frequency is a Monte Carlo
estimate of one functional, so the value of estimating it exactly is the infinite
draw limit of the accuracy curve. Fitting accuracy as $a - b/n$ across pool
depths from 5 to 100 gives a limit of 0.368 to 0.383, a headroom of 0.008 to
0.022 on the relation. Two disjoint fifty draw halves of the same pool disagree
on 38.1 percent of validation rows at almost no cost in accuracy, because the
values they disagree between are equally likely to be incorrect.

The available improvement is therefore not a better selector, and not a better
aggregator. On this relation the model is estimating within a plausible range,
and the range contains the answer often enough for coverage to appear to
...
smaller than that, including the apparent margins on \texttt{hasCapacity} and
\texttt{personHasCityOfDeath}, are point estimates rather than demonstrated
leads, and are reported as such.

Thresholds are selected using validation, so validation is a selection set rather
than a clean holdout, and the reported validation figure is optimistic by an
amount I estimate at 0.015 to 0.019.

The award relation is not meaningfully evaluable at ten rows and I claim no
result on it. The validation to test transfer estimates rest on a single edition
...
achieve; it is a diagnostic rather than a method. And the per relation choice of
numeric aggregator is supported by a confidence interval excluding zero only for
...
record, including the runs that failed, is in \texttt{docs/engineering-notes.md} in that
repository. All figures reported here are produced by the official
\texttt{evaluate.py} from \texttt{lm-kbc/dataset2026}, imported directly rather
than parsed from its printed output.
...
\paragraph{Cross-frame consensus scores below the best single frame.} The
hypothesis was that a value surviving a change of register is better evidence
than one that is merely modal within a single register. On identical cached
draws, consensus scored 0.2887 against 0.3196. The premise was incorrect in a
specific way: the frames share one base model and one prior toward round numbers,
so their errors are correlated. For a venue whose true capacity is 5000, three
frames independently proposed 10000, 10500 and 15000. Agreement among correlated
predictors provides little evidence.

\paragraph{Routing by confidence also scores lower.} Concentration of draws is
strongly predictive of correctness: accuracy rises from 0.000 at support below
0.2 to 0.886 above 0.8. But routing each subject to its most concentrated frame
scored 0.2784, and per frame calibrated concentration scored 0.3093, both below
the best single frame. The reason is structural: only about a quarter of
frame-subject pairs reach support above 0.6, and below that every frame sits near
0.15, so the choice is made precisely where no signal exists. An oracle selecting
the correct frame per subject would score 0.3814. The information is present and
three methods failed to extract it, which I record as an open problem rather than
a result.
...
All three were found by reading errors rather than by adding compute, and all
three are the kind of fault that leaves every self-check passing.

\paragraph{Integer rounding removed small areas.} The numeric writer ended by
...
one with the correct value already in its pool. Correcting the formatter moved
\texttt{hasArea} from 0.7600 to 0.8500, a gain of 0.019 on the overall metric,
larger than any modelling change made here.
...
draw proposed and no gold at either end accepts. I replaced it with a fixed
...
and it optimises the quantity the metric rewards.

\paragraph{A leakage guard correct for one channel and incorrect for two.}
Threshold tuning excludes a channel's own few shot demonstrations from its train
curve, since a demonstration subject has its gold answer in its own prompt. That
guard is sufficient for any single channel measurement and insufficient, without
this being visible, for any measurement reading two channels at once. Channels
draw different demonstration sets from the same 100 train rows, so a subject
clean for one frame is frequently a demonstration for another, and a cross frame
feature computed on it encodes gold rather than agreement. The union of
demonstration subjects is 64 of 100 train rows for \texttt{hasCapacity} and 74 of
100 for \texttt{hasArea}, reaching all 100 once the 100 shot frame is included,
...
validation does not detect this and neither does a label permutation control,
because both test whether the model is learning something real while the leak is
in the feature for that subject. The only check that detected it was asking which
