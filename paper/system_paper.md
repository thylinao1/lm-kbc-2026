...
STATUS: draft. Every number is measured and traceable to a logged run in
NOTES.local.md. Test figures come from the official grader over the 475-row
test set. Remaining work is reference formatting and a final pass once the last
...
no retrieval. Our starting observation is arithmetic rather than architectural:
...
published recipes cannot beat the leader by more than noise. Improvement has to
come from exceeding a per relation best.

The system scores 0.7060 and leads on five of six relations.
...
answer directly, moved hasArea from 0.8100 to 0.8700 and is also our best frame
for hasCapacity. Three earlier configurations of hasArea, across two frames and
two aggregation methods, had all returned exactly 0.8100, which we had read as
evidence that the answers were absent from the candidate pool. They were not.
...
coverage analyses report that hasCapacity, the weakest relation in the field, is
...
beyond the rank one answer we already emit falls from 0.443 to 0.203, and adding
...
We then test the conclusion with an instrument that is not a function of vote
frequency at all. Every selector in this literature, and all ten of ours, reads
the same sampled frequency table. Forced choice duels do not: we hand the model
...
We also report a set of negative results on closing the apparent gap. Cross-frame
...
simple thresholded frequency rule they replace. Finally we report three
implementation faults that each cost more than any modelling decision we made:
...
We also show that a single all-empty submission recovers every relation's test
...
on hasCapacity is gone. We report the retraction in full, including the effort we
...
The empty gold column is measured, not assumed. We submitted a single all empty
...
Two consequences we use throughout. awardWonBy is 2.1% of the score across ten
...
Two of its findings shaped our design: base completion beats instruction tuned
on most relations, and a standalone yes or no abstention gate was measurably
inert while removing vote share cost a large amount. We reimplement the recipes
...
is released under CC BY 4.0 and we credit it for the multi channel numeric idea
and the overshoot calibration.

From the 2025 edition we take entity level self consistency with per relation
...
relation. We also take the negative result that decomposition hurts medium
...
One model, gemma-4-31B, in base completion mode, served with vLLM. We measured
the checkpoint at 31,273,088,876 BF16 parameters from the safetensors index,
which is under the 32B cap. We note for auditors that the model's hosting page
...
We did not run a substrate comparison. The published measurements on the
...
For numeric relations we cluster the draws by single linkage in log space with a
...
environment variables on every inference job because the compute nodes we use
...
list and token limit per channel. We disable the JIT sampling kernel and use the
...
figure from 0.9854 to 0.9924. We exclude demonstration subjects from the train
curve. The cost is power: 35 scorable subjects remain, so train side threshold
choice on small relations is weak, and we say so rather than trusting it.
...
comes first in the grid, which is an arbitrary edge. We take the centre of the
...
A gate that conflated the two would reject the best performing channel we have,
...
| relation | rows | ours (test) | leader (test) | difference |
|---|---:|---:|---:|---:|
...
We lead on five of six relations. The exception, awardWonBy, has ten rows and a
relation-level confidence interval of roughly plus or minus 0.23, so it is not
measurable at this size and we did not spend effort on it.
...
figure of 0.8500. The same register is our best frame for hasCapacity, where it
...
exactly 0.8100 on test. We had concluded from that stability, and from the fact
...
inverted the sign of the remaining two. Both relations we had written off as
...
We report this as a methodological finding rather than a trick. Calibrating
scalar decision parameters on returned aggregate per-relation scores is
disclosed here as part of the method. We did not, and would not, use per-row
...
Every scalar axis we control is therefore at a measured local optimum, and
...
That reading is wrong, and the rest of this subsection is the correction. We
report it in full because we believed it for most of the campaign and spent most
of our effort on it.
...
half times smaller than the number we, and the published system we were chasing,
had been quoting.
...
We report three negative results, because the reasoning behind them is the more
useful contribution.
...
information is there; three methods failed to extract it. We record that as an
...
proposed and no gold at either end accepts. We replaced it with a fixed radius
...
**A leakage guard that was correct for one channel and wrong for two.** Our
...
All three exceed a five point shift, and they move in opposite directions. We
...
abstains well scores higher on test than on validation there. Our
...
entire field sits below 0.33. Read naively, our coverage analysis says 0.443 of
the relation is recoverable by better selection alone. We did not recover it, and
...
We tested that conclusion rather than resting on it. Forced choice duels are the
one instrument available to us that is not a transformation of the sampled
frequency table. We generated every pairwise comparison among the top six
...
0.7900 against 0.8500 on hasArea. It loses.
...
smaller than that, including our apparent margins on hasCapacity and
personHasCityOfDeath, are point estimates and not demonstrated leads. We say so
